...
class EnhancedT2IDataset(Dataset):
    ...
    def _load_data(self) -> List[Dict[str, Any]]:
        ...
        if self.manifest_path and self.manifest_path.exists():
            ...
            for img_path in self.data_path.rglob("*.jpg"):
                ...
                item = {"image_path": str(img_path), "caption": caption, "annotations": annotations}
                data_items.append(item)
    ...
    def _process_spatial_control(self, caption: str, annotations: Dict[str, Any], image: Image.Image) -> Dict[str, Any]:
        ...
        if object_layout is None:
            ...
            if objects:
                scene_objects = self.scene_composer.create_scene_layout(
                    objects[: self.max_objects], layout_type="balanced"
                )
                ...
                for i, obj in enumerate(scene_objects):
                    if i < self.max_objects:
                        layout_tensor[i] = torch.tensor([obj.position[0], obj.position[1], obj.size[0], obj.size[1]])
    ...
    def _process_anatomy_awareness(
        self, caption: str, annotations: Dict[str, Any], image: Image.Image
    ) -> Dict[str, Any]:
    ...
    def _process_consistency(self, annotations: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if character_name and character_name in self.character_to_id:
            consistency_data["character_id"] = torch.tensor(self.character_to_id[character_name], dtype=torch.long)

        # Style ID
        style_name = annotations.get("style_name")
        if style_name and style_name in self.style_to_id:
            consistency_data["style_id"] = torch.tensor(self.style_to_id[style_name], dtype=torch.long)
