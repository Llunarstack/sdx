...
3. **Caption dropout (and schedules)**  
   Randomly dropping or shortening captions forces the model to rely on latents + prior → more **inventive** fill-in; scheduled dropout is documented in [`docs/IMPROVEMENTS.md`](../research/IMPROVEMENTS.md) (`--caption-dropout-schedule`).
...
5. **Min-SNR, timestep sampling, EMA**  
   Stabilize learning so the model generalizes rather than fitting spurious caption–image quirks ([`docs/MODERN_DIFFUSION.md`](../research/MODERN_DIFFUSION.md)).
