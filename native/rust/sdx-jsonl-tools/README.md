...
Fast JSONL manifest **stats**, **validate**, **prompt-lint**, **`image-paths`**, **`dup-image-paths`**, plus **`file-fnv`** (raw-byte FNV-1a 64 + newline count) and **`caption-len-buckets`** (caption length histogram for trimming datasets).
