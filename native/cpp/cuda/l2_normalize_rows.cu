...
__global__ void sdx_k_l2_normalize_rows(const float *__restrict__ src, float *__restrict__ dst, int n_rows, int dim,
                                          float eps) {
    const int row = blockIdx.x * blockDim.x + threadIdx.x;
    if (row >= n_rows) {
        return;
    }
    const float *r = src + static_cast<size_t>(row) * dim;
    float *out = dst + static_cast<size_t>(row) * dim;
    float s = 0.f;
    for (int i = 0; i < dim; ++i) {
        const float v = r[i];
        s += v * v;
    }
    float inv = rsqrtf(fmaxf(s, eps * eps));
    for (int i = 0; i < dim; ++i) {
        out[i] = r[i] * inv;
    }
}

extern "C" int sdx_cuda_l2_normalize_rows_f32_host(float *host_data, int n_rows, int dim, float eps) {
