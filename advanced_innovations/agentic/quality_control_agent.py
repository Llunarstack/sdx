...
class QualityMetrics:
    ...
    semantic_consistency: float  # 0-1, consistency across encoders
...
class SemanticConsistencyAgent(nn.Module):
    ...
    def forward(self, encoder_features: List[torch.Tensor]) -> Tuple[float, float]:
        ...
        return consistency.mean().item(), divergence.mean().item()
...
class VisualQualityAgent(nn.Module):
    ...
    def forward(self, prompt_embedding: torch.Tensor) -> Dict[str, float]:
        ...
        aesthetic = self.aesthetic_scorer(prompt_embedding).mean().item()
        detail = self.detail_scorer(prompt_embedding).mean().item()
        realism = self.realism_scorer(prompt_embedding).mean().item()
...
class QualityControlSystem:
    ...
    def evaluate_generation(
        ...
        logger.info(
            ...
            f"Consistency: {assessment.semantic_consistency:.2%})"
        )
