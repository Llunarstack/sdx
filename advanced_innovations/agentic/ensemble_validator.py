...
class SemanticValidator(nn.Module):
    ...
    def forward(
        ...
        # Score alignment
        score = float(self.semantic_scorer(combined).squeeze())

        # Confidence
        confidence = float(self.confidence_predictor(combined).squeeze())
...
class DetailValidator(nn.Module):
    ...
    def forward(self, generated_embedding: torch.Tensor) -> ValidatorOutput:
        ...
        detail_score = float(self.detail_scorer(generated_embedding).squeeze())
        complexity = float(self.complexity_analyzer(generated_embedding).squeeze())
...
class AestheticValidator(nn.Module):
    ...
    def forward(self, generated_embedding: torch.Tensor) -> ValidatorOutput:
        ...
        aesthetic = float(self.aesthetic_scorer(generated_embedding).squeeze())
        harmony = float(self.harmony_checker(generated_embedding).squeeze())
...
class ConsistencyValidator(nn.Module):
    ...
    def forward(
        ...
        # Score consistency
        score = float(self.consistency_scorer(combined).squeeze())
...
class RealisticValidator(nn.Module):
    ...
    def forward(self, generated_embedding: torch.Tensor) -> ValidatorOutput:
        ...
        realism = float(self.realism_scorer(generated_embedding).squeeze())
        artifact_score = float(self.artifact_detector(generated_embedding).squeeze())
