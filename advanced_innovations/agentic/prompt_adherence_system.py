...
class PromptAnalysis:
    ...
    color_palette: List[str]  # Colors mentioned
...
class PentaEncoderSemanticAnalyzer(nn.Module):
    ...
    def __init__(self):
        ...
        # Semantic parser
        self.semantic_parser = nn.Sequential(
    ...
    def forward(
        ...
        semantic = self.semantic_parser(combined)
...
class PromptValidator(nn.Module):
    ...
    def forward(
        ...
        return {
            "semantic": semantic_match.mean().item(),
            "detail": detail_match.mean().item(),
            "style": style_match.mean().item(),
            "composition": composition_match.mean().item(),
            "overall": (
                semantic_match.mean().item() * 0.4
                + detail_match.mean().item() * 0.2
                + style_match.mean().item() * 0.2
                + composition_match.mean().item() * 0.2
            ),
        }
...
class DynamicPromptEnforcer(nn.Module):
    ...
    def forward(
        ...
        return {
            "guidance_scale": float(guidance.mean().item()),
            "temperature": float(torch.clamp(temperature, 0.1, 1.0).mean().item()),
            "refinement_strength": float(refinement.mean().item()),
        }
