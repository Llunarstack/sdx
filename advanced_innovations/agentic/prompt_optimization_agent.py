...
class PromptAnalyzer(nn.Module):
    ...
    def forward(self, embedding: torch.Tensor, original_text: str = "") -> PromptAnalysis:
        ...
        # Score aspects
        coverage = float(self.coverage_scorer(features).squeeze())
        vagueness = float(self.vagueness_detector(features).squeeze())
        specificity = float(self.specificity_scorer(features).squeeze())
        technical = float(self.technical_analyzer(features).squeeze())
        clarity = (specificity + (1 - vagueness)) / 2

        # Detect missing aspects
        aspect_logits = self.aspect_detector(features)
        aspect_scores = torch.sigmoid(aspect_logits[0])
        missing = []
        ...
        if float(aspect_logits[0][0]) < 0.3:  # lighting
