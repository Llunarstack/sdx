...
class LPIPSMetric(nn.Module):
    ...
    def forward(
        ...
        lpips = 1.0 - float(similarity.squeeze())
...
class DINOMetric(nn.Module):
    ...
    def forward(
        ...
        # Final score
        dino_score = (
            float(similarity.squeeze()) * 0.7 +
            (float(coherence_a.squeeze()) + float(coherence_b.squeeze())) / 2 * 0.3
        )
...
class DreamSimMetric(nn.Module):
    ...
    def forward(
        ...
        dreamsim_score = float(similarity.squeeze())
