...
def main() -> int:
    from utils.architecture.ar_block_conditioning import ar_conditioning_vector, parse_num_ar_blocks_from_row

    from vit_quality.checkpoint_utils import load_vit_quality_checkpoint
    from vit_quality.dataset import text_feature_vector
    from vit_quality.tta import tta_predict
    ...
    with inp.open("r", encoding="utf-8") as rf, outp.open("w", encoding="utf-8") as wf:
        for line in rf:
            ...
            wf.write(json_dumps(row, ensure_ascii=False) + "\n")
            n += 1
