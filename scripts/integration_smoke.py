...
def _run_step(report: SmokeReport, name: str, fn: Callable[[], Optional[str]], *, skip: bool = False) -> None:
    if skip:
...
def _frame_count_by_site(data_root: Path) -> dict[str, int]:
    ...
    for site in ALL_SCRAPE_SITES:
        out[site] = sum(
            1 for r in _read_manifest_rows(data_root / site / "manifest.jsonl") if r.get("parent_md5")
        )
    return out
...
def _pytest(patterns: list[str]) -> str:
    patterns = patterns or ["tests/test_image_profiler.py", "tests/test_reverse_search.py", "tests/test_data_pipeline.py"]
    r = subprocess.run(
...
def main(argv: list[str] | None = None) -> int:
    ...
    report = SmokeReport()
    print(f"SDX integration smoke", flush=True)
    ...
    def scrape_animated_frames() -> str:
        ...
        if still_missing:
            raise RuntimeError(
                f"no frame rows for sites={still_missing} after animated/gif/video fallbacks"
            )
    ...
    def verify_frame_splits() -> str:
        ...
        parents = {
            site: len({r.get("parent_md5") for r in _read_manifest_rows(data_root / site / "manifest.jsonl") if r.get("parent_md5")})
            for site in ALL_SCRAPE_SITES
        }
    ...
    def smoke_dataset() -> str:
        return _py(["-m", "scripts.tools", "make_smoke_dataset", "--out", str(data_root / "smoke_tiny"), "--count", "4", "--size", "128"])
    ...
    def mini_train_manifest() -> str:
        ...
        if not _t5_ready():
            raise RuntimeError(
                "T5-XXL not cached — run bash runpod/download.sh --models-only first"
            )
        man = _training_manifest(combined, enriched)
        if not man.is_file():
