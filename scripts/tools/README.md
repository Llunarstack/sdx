...
| `normalize_captions`, `preview_generation_prompt`, `vit_inspect`, `seed_explorer`, `eval_prompts`, `training_timestep_preview`, `image_quality_qc`, `dit_variant_compare`, `make_smoke_dataset`, `spatial_coverage`, `complex_prompt_coverage`, `prompt_gap_scout`, `book_scene_split`, `edit_inpaint` | top-level `scripts/tools/` single-file utilities (dispatcher-enabled). |
| `caption_hygiene`, `ar_tag_manifest` | `data/` (JSONL hygiene and AR block tagging). |
| `preview_prompt_stack` | Full PromptStack preview (trace, clauses, JSON). |
| `fetch_danbooru_tags`, `download_all_danbooru_categorized_tags`, `split_danbooru_general_tags`, `merge_danbooru_categorized_tags` | Danbooru tag curation (dispatcher-enabled). |
