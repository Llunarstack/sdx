...
def generic_suggestion(category: str, examples: List[str], lang: str) -> str:
    ...
    templates = {
        "en": "Add related keyword terms for '{category}' early in the caption and keep it consistent across the dataset."
        + ex_part,
        "es": "Para reforzar '{category}', añade términos relacionados al inicio del caption y mantén la consistencia en todo el dataset."
        + ex_part,
        "fr": "Pour renforcer '{category}', ajoutez des termes liés dès le début des captions et gardez la cohérence dans tout le dataset."
        + ex_part,
        "ru": "Чтобы усилить '{category}', добавляйте связанные термины в начале caption и сохраняйте единообразие по всему датасету."
        + ex_part,
        "ja": "'{category}' を強化するには、キャプションの早い段階に関連キーワードを入れ、データセット全体で一貫させてください。"
        + ex_part,
        ...
        "ta": "'{category}' யை வலுப்படுத்த, கேப்ஷனின் ஆரம்பத்திலேயே தொடர்புடைய முக்கிய சொற்களை சேர்த்து, முழு டேட்டாசெட்டிலும் ஒரே மாதிரி வைத்திருங்கள்。"
        + ex_part,
        ...
        "kn": "'{category}' ಅನ್ನು ಬಲಪಡಿಸಲು, ಕ್ಯಾಪ್ಷನ್‌ನ ಆರಂಭದಲ್ಲೇ ಸಂಬಂಧಿತ ಕೀವರ್ಡ್‌ಗಳನ್ನು ಸೇರಿಸಿ ಮತ್ತು ಸಂಪೂರ್ಣ ಡೇಟಾಸೆಟ್‌ನಲ್ಲಿ ಏಕತೆ ಇಟ್ಟುಕೊಳ್ಳಿ。"
        + ex_part,
        ...
        "ur": "‘{category}’ کو بہتر کرنے کے لیے کیپشن کے شروع میں متعلقہ keywords شامل کریں اور پورے ڈیٹاسیٹ میں مستقل مزاجی رکھیں۔"
        + ex_part,
        "it": "Per rafforzare '{category}', aggiungi termini correlati all'inizio dei caption e mantieni coerenza in tutto il dataset."
        + ex_part,
        "ar": "لتقوية '{category}'، أضف كلمات مفتاحية مرتبطة في بداية الـcaption واحتفظ بالاتساق عبر مجموعة البيانات كلها."
        + ex_part,
        "la": "Ad '{category}' roborandum, adde vocabula coniuncta primo in captione et eadem manere per totam tuam materiam."
        + ex_part,
    }
