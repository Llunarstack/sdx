...
class ConsistencyValidator:
    ...
    def validate_character_consistency(self, 
                                     ...
                                     character_profile: CharacterProfile) -> Dict[str, float]:
        """
        ...
        if face_region is not None and character_profile.face_embedding is not None:
            with torch.no_grad():
                ...
                face_similarity = self._cosine_similarity(
                    face_embedding.detach().cpu().numpy(),
                    character_profile.face_embedding.reshape(1, -1)
                )
        ...
        if body_region is not None and character_profile.body_embedding is not None:
            with torch.no_grad():
                ...
                body_similarity = self._cosine_similarity(
                    body_embedding.detach().cpu().numpy(),
                    character_profile.body_embedding.reshape(1, -1)
                )
...
class CharacterDatabase:
    ...
    def create_character(self, 
                        ...
                        style_preferences: Optional[StylePreferences] = None) -> CharacterProfile:
        """
