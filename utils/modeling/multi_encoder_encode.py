...
def prepare_multi_encoder_kwargs(
    ...
    *,
    layout_specs: list[LayoutSpec | None] | None = None,
    flat_full_prompts: list[str] | None = None,
) -> dict[str, list[str]]:
    """
    ...
    specs: list[LayoutSpec | None] = list(layout_specs) if layout_specs is not None else [None] * n
    if len(specs) < n:
