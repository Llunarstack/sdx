...
def repo_root() -> Path:
    ...
    # breaks whenever this module moves to a different depth.
...
def _folder_has_weights(local_model_path: Path, weight_globs: Sequence[str] | None = None) -> bool:
    """True when a local pretrained folder looks like it has checkpoint blobs."""
    if not local_model_path.is_dir():
        return False
    patterns = list(weight_globs or ("*.safetensors", "*.bin", "*.pt", "*.pth", "*.ckpt"))
    try:
        for pat in patterns:
            if any(local_model_path.glob(pat)):
                return True
            # Some hubs nest weights (e.g. transformer/*.safetensors).
            if any(local_model_path.rglob(pat)):
                return True
    except OSError:
        return False
    return False
...
def resolve_prefer(folder_names: Sequence[str], hf_fallback: str) -> str:
    """
    Prefer the first local folder in ``folder_names`` that has **weights**.

    Config-only scaffolds (tokenizer/config JSON) do not win — so upgrading
    ``pretrained/`` with small metadata folders won't shadow a legacy folder
    that still has real checkpoints. Falls back to the hub id when nothing
    local has weights.
    """
    for folder_name in folder_names:
        local_model_path = model_dir() / folder_name
        if _folder_has_weights(local_model_path):
            return str(local_model_path)
    return hf_fallback
...
def resolve_model_path_require_weights(
    ...
    hf_fallback: str,
    *,
    weight_globs: Sequence[str] | None = None,
) -> str:
    """
    ...
    """
    local_model_path = model_dir() / folder_name
    if _folder_has_weights(local_model_path, weight_globs):
        return str(local_model_path)
    return hf_fallback
...
def default_dinov2_large_path() -> str:
    """Legacy DINOv2-L path (architecture-compatible callers)."""
...
def default_dinov3_vitl16_path() -> str:
    """DINOv3 ViT-L/16 — preferred dense vision backbone (REPA / critics)."""
    return resolve_prefer(
        ("DINOv3-ViT-L16", "DINOv3-ViT-Large"),
        "facebook/dinov3-vitl16-pretrain-lvd1689m",
    )


def default_repa_vision_path() -> str:
    """Vision teacher for REPA / feature alignment — prefer DINOv3-L, else DINOv2-L."""
    return resolve_prefer(
        ("DINOv3-ViT-L16", "DINOv3-ViT-Large", "DINOv2-Large"),
        "facebook/dinov3-vitl16-pretrain-lvd1689m",
    )


def default_siglip_path() -> str:
    """Prefer SigLIP2; fall back to SigLIP SO400M local folder if present."""
    return resolve_prefer(
        ("SigLIP2-SO400M", "SigLIP-SO400M"),
...
def default_qwen_path() -> str:
    """
    Primary causal LLM for prompt expansion / RAG / style invent.

    Prefer Qwen3-14B (2025+). Legacy ``Qwen2.5-14B-Instruct`` local folders still win
    if present so existing installs keep working without a re-download.
    """
    return resolve_prefer(
        ("Qwen3-14B", "Qwen3-14B-Instruct", "Qwen2.5-14B-Instruct"),
        "Qwen/Qwen3-14B",
    )


def default_qwen3_text_encoder_path() -> str:
    """
    Best practical Qwen text encoder for modern T2I stacks (FLUX.2 Klein / Qwen-Image style).

    Industry default in 2025–2026 Comfy/Diffusers Klein workflows: ``Qwen/Qwen3-8B``.
    """
    return resolve_prefer(
        ("Qwen3-8B", "Qwen3-8B-Instruct", "Qwen3-4B"),
        "Qwen/Qwen3-8B",
    )


def default_qwen_vl_path() -> str:
    """Primary VLM — prefer Qwen3-VL-8B-Instruct over Qwen2.5-VL-7B."""
    return resolve_prefer(
        (
...
def default_depth_anything_v2_large_path() -> str:
    """Legacy Depth Anything V2; prefer ``default_depth_anything_v3_large_path`` for new work."""
...
def default_depth_anything_path() -> str:
    """Primary depth model — prefer DA3, else V2 Large local/hub."""
    return resolve_prefer(
        (
...
def default_moondream2_path() -> str:
    """Prefer Moondream 3 preview; fall back to moondream2 local/hub."""
    return resolve_prefer(
        ("moondream3-preview", "moondream3", "moondream2"),
        "moondream/moondream3-preview",
    )


# Preferred name — same resolver as legacy ``default_moondream2_path``.
default_moondream_path = default_moondream2_path
...
def default_hpsv2_path() -> str:
    """Prefer HPSv3 (ICCV 2025); fall back to HPSv2 local/hub."""
    return resolve_prefer(
        ("HPSv3", "HPSv2-hf", "HPSv2"),
        "MizzenAI/HPSv3",
    )


# Preferred name — same resolver as legacy ``default_hpsv2_path``.
default_hps_path = default_hpsv2_path


def default_hpsv3_path() -> str:
    return resolve_model_path_require_weights("HPSv3", "MizzenAI/HPSv3")


def default_depth_anything_v3_large_path() -> str:
    """Depth Anything 3 Large (2025+) — preferred depth scaffold."""
    return resolve_prefer(
        ("Depth-Anything-V3-Large", "DA3-LARGE-1.1", "DA3-LARGE"),
        "depth-anything/DA3-LARGE-1.1",
    )


def default_flux2_klein_path() -> str:
    """External FLUX.2 Klein foundation (sibling Diffusers / Comfy stack)."""
    return resolve_prefer(
        ("FLUX.2-klein-9B", "FLUX.2-klein-4B", "FLUX.2-dev"),
        "black-forest-labs/FLUX.2-klein-9B",
    )


def default_dc_ae_path() -> str:
    """Deep Compression Autoencoder (DC-AE) for modern DiT latent experiments."""
    return resolve_prefer(
        ("DC-AE-f32c32", "dc-ae-f32c32-mix-1.0"),
        "mit-han-lab/dc-ae-f32c32-mix-1.0",
    )


def default_internvl3_path() -> str:
    return resolve_prefer(
        ("InternVL3-8B", "InternVL3-8B-hf"),
        "OpenGVLab/InternVL3-8B-hf",
    )


def default_gemma3_vl_path() -> str:
    return resolve_prefer(
...
def default_qwen25_vl_7b_path() -> str:
    """Legacy name — resolves to best available Qwen VL (prefers Qwen3-VL-8B)."""
    return default_qwen_vl_path()
...
def pretrained_catalog() -> list[dict[str, str]]:
    ...
    catalog_rows = [
        ...
        ("Qwen3-8B", "Qwen/Qwen3-8B", default_qwen3_text_encoder_path()),
        ("Qwen3-14B", "Qwen/Qwen3-14B", default_qwen_path()),
        ("Qwen3-VL-8B-Instruct", "Qwen/Qwen3-VL-8B-Instruct", default_qwen_vl_path()),
        ("DINOv3-ViT-L16", "facebook/dinov3-vitl16-pretrain-lvd1689m", default_dinov3_vitl16_path()),
        ("DINOv2-Large", "facebook/dinov2-large", default_dinov2_large_path()),
        ("DINOv2-Giant", "facebook/dinov2-giant", default_dinov2_giant_path()),
        ("SigLIP2-SO400M", "google/siglip2-so400m-patch16-384", default_siglip_path()),
        (
            "SigLIP-SO400M",
            "google/siglip-so400m-patch14-384",
            resolve_model_path("SigLIP-SO400M", "google/siglip-so400m-patch14-384"),
        ),
        ("Depth-Anything-V3-Large", "depth-anything/DA3-LARGE-1.1", default_depth_anything_v3_large_path()),
        ("FLUX.2-klein-9B", "black-forest-labs/FLUX.2-klein-9B", default_flux2_klein_path()),
        ("DC-AE-f32c32", "mit-han-lab/dc-ae-f32c32-mix-1.0", default_dc_ae_path()),
        ("InternVL3-8B", "OpenGVLab/InternVL3-8B-hf", default_internvl3_path()),
        ("Gemma-3-4B-IT", "google/gemma-3-4b-it", default_gemma3_vl_path()),
        (
            "Qwen2.5-14B-Instruct",
            "Qwen/Qwen2.5-14B-Instruct",
            resolve_model_path("Qwen2.5-14B-Instruct", "Qwen/Qwen2.5-14B-Instruct"),
        ),
        ...
        ("moondream3-preview", "moondream/moondream3-preview", default_moondream2_path()),
        ("moondream2", "vikhyatk/moondream2", resolve_model_path("moondream2", "vikhyatk/moondream2")),
        ...
        ("HPSv3", "MizzenAI/HPSv3", default_hpsv3_path()),
        ("HPSv2-hf", "adams-story/HPSv2-hf", resolve_model_path_require_weights("HPSv2-hf", "adams-story/HPSv2-hf")),
