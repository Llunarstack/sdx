...
def print_model_summary(model: nn.Module, input_shape: Tuple[int, ...] = None):
    ...
    print("-" * 50)
    for module_type, count in sorted(analysis["parameter_counts_by_type"].items(), key=lambda x: x[1], reverse=True):
...
def get_layer_wise_lr_groups(model: nn.Module, base_lr: float = 1e-4) -> List[Dict]:
    ...
    for depth in sorted(layer_groups.keys()):
        ...
        param_groups.append({"params": layer_groups[depth], "lr": group_lr, "layer_depth": depth})
...
def estimate_training_memory(
    model: nn.Module, batch_size: int, sequence_length: int, use_gradient_checkpointing: bool = False
) -> Dict[str, float]:
