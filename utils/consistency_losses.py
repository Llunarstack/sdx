...
class CharacterConsistencyLoss(nn.Module):
    ...
    def forward(self, 
                ...
                reference_images: Optional[torch.Tensor] = None) -> Dict[str, torch.Tensor]:
        """
    ...
    def _compute_facial_consistency_loss(self, 
                                       generated_images: torch.Tensor,
    ...
    def _compute_body_consistency_loss(self,
                                     generated_images: torch.Tensor,
                                     character_profiles: List[CharacterProfile]) -> torch.Tensor:
    ...
    def _compute_color_consistency_loss(self,
                                      generated_images: torch.Tensor,
                                      character_profiles: List[CharacterProfile]) -> torch.Tensor:
        ...
        for i, profile in enumerate(character_profiles):
            ...
            palette_colors = torch.tensor([
                self._color_name_to_rgb(color) for color in color_palette
    ...
    def _compute_style_consistency_loss(self,
                                      generated_images: torch.Tensor,
                                      character_profiles: List[CharacterProfile]) -> torch.Tensor:
    ...
    def _compute_reference_similarity_loss(self,
                                         generated_images: torch.Tensor,
...
class StyleInvariantLoss(nn.Module):
    ...
    def forward(self, 
                style1_embeddings: torch.Tensor,
                style2_embeddings: torch.Tensor) -> torch.Tensor:
        """
...
def create_consistency_loss_system(face_encoder: FaceEncoder, 
                                 body_encoder: BodyEncoder) -> Dict[str, nn.Module]:
