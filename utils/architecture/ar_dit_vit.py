...
JSONL fields (first match wins in ``parse_num_ar_blocks_from_row``):

- ``num_ar_blocks``, ``dit_num_ar_blocks``, ``ar_blocks``, ``generator_num_ar_blocks``
- Optional nested (depth-limited, default 2): ``dit_config``, ``train_config``, ``checkpoint_config``, ``model_config``

See ``models/attention.py`` ``create_block_causal_mask_2d``.
"""
...
def ar_bridge_summary_lines() -> str:
    ...
    return (
        "DiT num_ar_blocks: 0=full bidirectional (default Meta DiT-style), 2=2×2 block AR, 4=4×4 block AR. "
