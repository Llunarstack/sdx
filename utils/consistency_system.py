...
class ConsistencyManager:
    ...
    def _load_styles(self) -> Dict[str, StyleProfile]:
        ...
        if self.styles_file.exists():
            with open(self.styles_file, 'r') as f:
                data = json.load(f)
                return {
                    name: StyleProfile(**profile_data)
                    for name, profile_data in data.items()
                }
        return {}
    ...
    def _load_scenes(self) -> Dict[str, SceneContext]:
        ...
        if self.scenes_file.exists():
            with open(self.scenes_file, 'r') as f:
                data = json.load(f)
                return {
                    scene_id: SceneContext(**scene_data)
    ...
    def save_all(self):
        ...
        with open(self.styles_file, 'w') as f:
            json.dump({
                name: asdict(profile)
                for name, profile in self.styles.items()
            }, f, indent=2)
        
        # Save scenes
        with open(self.scenes_file, 'w') as f:
            json.dump({
                scene_id: asdict(context)
                for scene_id, context in self.scenes.items()
            }, f, indent=2)
    
    def create_character_profile(self, name: str, description: str,
                               reference_prompt: str = None) -> CharacterProfile:
    ...
    def create_style_profile(self, name: str, style_description: str,
                           reference_prompt: str = None) -> StyleProfile:
    ...
    def create_scene_context(self, scene_id: str, location: str,
                           base_prompt: str) -> SceneContext:
    ...
    def generate_consistent_prompt(self, base_prompt: str,
    ...
    def _extract_style_tags(self, description: str) -> List[str]:
        ...
        style_keywords = [
            "realistic", "anime", "cartoon", "photorealistic", "artistic",
            "detailed", "simple", "complex", "stylized", "natural"
        ]
    ...
    def _extract_artistic_elements(self, description: str) -> List[str]:
        ...
        elements = []
        artistic_terms = [
            "painterly", "sketchy", "smooth", "textured", "detailed",
            "minimalist", "ornate", "geometric", "organic", "abstract",
            "realistic", "stylized", "vintage", "modern", "classical"
        ]
    ...
    def _extract_objects(self, prompt: str) -> List[str]:
        ...
        common_objects = [
            "tree", "car", "house", "chair", "table", "book", "flower",
            "mountain", "river", "building", "bridge", "road", "window"
        ]
...
class ReferenceImageManager:
    ...
    def save_reference_image(self, image: Image.Image, 
                           reference_id: str, 
