...
class PreferenceMemory(nn.Module):
    ...
    def update_profile(
        ...
        # Extract affinities (detached: used only as scalar preference signals, not in any loss)
        subject_aff = self.subject_affinity(encoded).detach()
        style_aff = self.style_affinity(encoded).detach()
        mood_aff = self.mood_affinity(encoded).detach()
...
class ThemeAnalyzer(nn.Module):
    ...
    def extract_themes(
        ...
        if combined is not None and total_weight > 0:
            ...
            theme_scores = torch.softmax(theme_logits[0], dim=0).detach()
