...
class RewardModel(nn.Module):
    ...
    def forward(self, image_features: torch.Tensor) -> Tuple[float, float]:
        ...
        reward = float(self.reward_head(encoded).squeeze().detach())
        confidence = float(self.confidence_head(encoded).squeeze().detach())
...
class PreferenceOptimizer(nn.Module):
    ...
    def estimate_value(self, image_features: torch.Tensor) -> float:
        ...
        value = float(self.value_net(image_features).squeeze().detach())
        return value
...
class RLHFAgent:
    ...
    def get_policy_improvements(self, current_image_features: torch.Tensor) -> Dict:
        ...
        adjustments_normalized = torch.sigmoid(adjustments).detach()
...
if __name__ == "__main__":
    ...
    for i in range(15):
        ...
        confidence = 0.7 + torch.rand(1).detach().item() * 0.3
