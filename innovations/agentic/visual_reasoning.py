    ...
    def forward(self, embedding: torch.Tensor) -> List[VisualConcept]:
        ...
        for idx, score in zip(top_concepts.indices, top_concepts.values):
            ...
            concept = VisualConcept(
                ...
                confidence=float(confidence.mean().detach()),
                spatial_location=tuple(float(x) for x in spatial[0].detach()),
...
class SceneUnderstandingEngine(nn.Module):
    ...
    def forward(
        ...
        color_temp = float(self.color_temp_analyzer(embedding).mean().detach())
