...
class ParameterOptimizer(nn.Module):
    ...
    def forward(
        ...
        guidance_raw = float(self.guidance_optimizer(guidance_input).squeeze().detach())
        ...
        temperature = 0.3 + (float(temperature.detach()) * 0.5)  # 0.3-0.8 range
        ...
        refinement = float(refinement.detach())
