...
class GANArtifactDetector(nn.Module):
    ...
    def forward(self, image_features: torch.Tensor) -> Dict:
        ...
        checkerboard_score = float(self.checkerboard_detector(image_features).squeeze().detach())

        # Detect mode collapse
        mode_collapse_score = float(self.mode_collapse_detector(image_features).squeeze().detach())

        # Detect frequency artifacts
        frequency_score = float(self.frequency_detector(image_features).squeeze().detach())
...
class DiffusionArtifactDetector(nn.Module):
    ...
    def forward(
        ...
        # Detect speckles
        speckle_score = float(self.speckle_detector(image_features).squeeze().detach())

        # Detect color banding
        banding_score = float(self.banding_detector(image_features).squeeze().detach())

        # Detect over-smoothing
        smoothing_score = float(self.smoothing_detector(image_features).squeeze().detach())
        ...
        if prev_frame is not None:
            ...
            temporal_score = float(self.temporal_inconsistency_detector(combined).squeeze().detach())
