...
class AestheticQualityModule(nn.Module):
    ...
    def forward(self, image_features: torch.Tensor) -> PreferenceDimension:
        ...
        composition = float(self.composition_scorer(image_features).squeeze().detach())
        color = float(self.color_harmony_scorer(image_features).squeeze().detach())
        lighting = float(self.lighting_scorer(image_features).squeeze().detach())
...
class DetailRichnessModule(nn.Module):
    ...
    def forward(self, image_features: torch.Tensor) -> PreferenceDimension:
        ...
        texture = float(self.texture_scorer(image_features).squeeze().detach())
        sharpness = float(self.sharpness_scorer(image_features).squeeze().detach())
...
class SemanticAlignmentModule(nn.Module):
    ...
    def forward(
        ...
        subject = float(self.subject_alignment(pf).squeeze().detach())
        style = float(self.style_alignment(imf).squeeze().detach())
...
class TechnicalExcellenceModule(nn.Module):
    ...
    def forward(self, image_features: torch.Tensor) -> PreferenceDimension:
        ...
        artifacts = 1.0 - float(self.artifact_detector(image_features).squeeze().detach())
        distortion = 1.0 - float(self.distortion_detector(image_features).squeeze().detach())
...
class EmotionalImpactModule(nn.Module):
    ...
    def forward(self, image_features: torch.Tensor) -> PreferenceDimension:
        ...
        mood = float(self.mood_scorer(image_features).squeeze().detach())
        atmosphere = float(self.atmosphere_scorer(image_features).squeeze().detach())
