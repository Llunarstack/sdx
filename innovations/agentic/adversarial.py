...
class RobustnessEvaluator(nn.Module):
    ...
    def forward(
        ...
        drift = float(self.drift_detector(combined).squeeze().detach())

        # Score stability (how stable the generation is)
        stability = float(self.stability_scorer(perturbed_embedding).squeeze().detach())
...
if __name__ == "__main__":
    ...
    def mock_scoring_func(embedding):
        return torch.rand(1).detach().item() * 0.95
