"""
Tests for ``diffusion/sampling_extras`` (advanced sampling helpers).

Verifies per-step plans, presets, runtime guard clamps, and latent refinement helpers.

Run with:
    pytest tests/test_sampling_extras.py -v
"""
...
class TestHolyGrailBlueprint:
    def test_step_plan_at_start(self):
        from diffusion.sampling_extras.blueprint import HolyGrailRecipe, build_holy_grail_step_plan
...
class TestHolyGrailPresets:
    def test_list_presets(self):
        from diffusion.sampling_extras.presets import list_holy_grail_presets
    ...
    def test_get_preset(self):
        from diffusion.sampling_extras.presets import get_holy_grail_preset
    ...
    def test_preset_values_sane(self):
        from diffusion.sampling_extras.presets import HOLY_GRAIL_PRESETS
...
class TestRuntimeGuard:
    def test_sanitize_clamps_cfg_ratios(self):
        from diffusion.sampling_extras.runtime_guard import sanitize_holy_grail_kwargs
...
class TestConditionAnnealing:
    def test_cads_noise_std_at_start(self):
        from diffusion.sampling_extras.condition_annealing import cads_noise_std
    ...
    def test_apply_condition_noise_shape(self):
        from diffusion.sampling_extras.condition_annealing import apply_condition_noise
...
class TestLatentRefiner:
    def test_unsharp_mask_shape(self):
        from diffusion.sampling_extras.latent_refiner import unsharp_mask_latent
    ...
    def test_dynamic_percentile_clamp_shape(self):
        from diffusion.sampling_extras.latent_refiner import dynamic_percentile_clamp
        x = torch.randn(2, 4, 16, 16)
    ...
    def test_consistency_blend_shape(self):
        from diffusion.sampling_extras.latent_refiner import consistency_blend_latent
        x = torch.randn(2, 4, 16, 16)
...
class TestRecommender:
    def test_anime_prompt(self):
        from diffusion.sampling_extras.recommender import recommend_holy_grail_preset
