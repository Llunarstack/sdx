...
- **`data/caption_utils.prepend_quality_if_short`** — when `--book-accuracy balanced|maximum` or `--prepend-quality-if-short`.
...
Use `--book-accuracy maximum` for more candidates (slower) and stronger post-process defaults.
