...
def cmd_optimize_prompt(args):
    ...
    if args.prompt:
        ...
        optimized = optimizer.optimize_prompt(
            ...
            boost_subject=not args.no_boost
        )
...
def cmd_generate_image(args):
    ...
    try:
        ...
        if args.harmonize_styles and (lora_configs or any(style_word in args.prompt.lower() 
                                                         for style_word in ['anime', 'realistic', 'cartoon', '3d', 'painting'])):
...
def cmd_create_character_profile(args):
    ...
    style_preferences = StylePreferences(
        clothing_style=args.clothing_style,
...
def cmd_list_characters(args):
    ...
    for char in characters:
        ...
        print(f"{char.character_id:<12} {char.name:<20} {len(char.reference_images):<5} "
...
def cmd_character_statistics(args):
    ...
    characters_with_embeddings = sum(1 for char in characters 
                                   if char.face_embedding is not None or char.body_embedding is not None)
...
def main():
    ...
    merge_parser.add_argument('--method', choices=['weighted_average', 'max', 'min'], 
    ...
    generate_parser.add_argument('--harmonization-mode', choices=['balanced', 'preserve_dominant', 'blend_all'], 
                                default='balanced', help='Style harmonization mode')
    ...
    char_create_parser.add_argument('--face-shape', choices=['oval', 'round', 'square', 'heart', 'diamond'], default='oval')
    ...
    style_harmonize_parser.add_argument('--mode', choices=['balanced', 'preserve_dominant', 'blend_all'], 
                                       default='balanced', help='Harmonization mode')
    style_harmonize_parser.add_argument('--max-reduction', type=float, default=0.5, 
                                       help='Maximum strength reduction allowed')
    style_harmonize_parser.add_argument('--allow-prompt-changes', action='store_true', 
                                       help='Allow modifications to the prompt')
