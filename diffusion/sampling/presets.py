"""Named, pre-tuned configurations of the "holy grail" sampling recipe.

Each ``HolyGrailPreset`` bundles a full set of recipe knobs (CFG ramp, control/adapter
multipliers, CADS condition annealing, unsharp masking, latent clamping) tuned for a
content type: ``balanced``, ``photoreal``, ``anime``, ``illustration``, ``aggressive``.

``apply_holy_grail_preset_to_args`` applies a preset *softly* — it only overrides args
that are still at their defaults, so explicit user choices on the CLI always win.
See ``blueprint.py`` for what these knobs actually do during sampling.
"""
...
class HolyGrailPreset:
    """One tuned recipe configuration (see module docstring for the knobs)."""
