"""The "holy grail" sampling recipe — core data model and per-step planner.

"Holy grail" is this repo's name for a coordinated sampling recipe that chases the
usual hard goal: strong prompt adherence *and* stable style/control, without the
quality or diversity collapse you normally trade for it. Rather than a single fixed
CFG and fixed control/adapter strengths, it varies all of them together over the
denoise trajectory.

Flow:
  ``HolyGrailRecipe``  – the static, user-facing configuration (ratios, base scales).
  ``build_holy_grail_step_plan`` – turns the recipe + current step into a
      ``HolyGrailStepPlan`` of concrete scalars for *this* step (CFG ramps up,
      control front-loads, adapters boost late, refinement strengthens late).
  ``apply_step_plan_to_kwargs`` – writes those scalars into the model kwargs.

Tuned named configurations live in ``presets.py``; ``runtime_guard.py`` clamps the
knobs to safe ranges.
"""
