"""Runtime safety guard for "holy grail" sampling parameters.

The holy-grail recipe exposes many tuning knobs (CFG ratios, control/adapter
multipliers, CADS noise, unsharp masking, latent clamping). Bad or contradictory
values can produce garbage or unstable samples. ``sanitize_holy_grail_kwargs``
clamps every knob to a known-stable range and enforces cross-parameter consistency
(e.g. CADS min can't exceed CADS strength), so callers can pass user input directly
without each sampler re-validating. Returns a copy; never mutates the input.
"""
