"""Attention-driven guidance fusion helpers for sampling.

Two ideas live here:

1. **Adaptive CFG from attention entropy** — when the model's attention is diffuse
   (high entropy) it is "unsure", so nudging guidance up helps; when attention is
   sharply focused (low entropy) guidance can ease off to preserve detail.
2. **Control/adapter scale fusion** — blends ControlNet-style control strength and
   adapter strength across the denoise trajectory (e.g. front-load structural
   control early, let adapters take over later).

Used by the samplers in ``gaussian_diffusion.py`` (notably the "holy grail" recipe).
"""
