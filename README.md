...
| **Content controls → CSV tag packs** | Prompt tag banks live under **[`data/prompt_tags/`](data/prompt_tags/)** (numbered `01_…`–`09_…` **`.csv`**, columns `pack`, `mode`, `tag`; **`__REF__:best_model_tags`** splices another pack). Loader: [`utils/prompt/content_control_tag_data.py`](utils/prompt/content_control_tag_data.py) · constants: [`utils/prompt/content_control_tags.py`](utils/prompt/content_control_tags.py) · API + inference: [`utils/prompt/content_controls.py`](utils/prompt/content_controls.py). **Regenerate** from loaded constants: **`python scripts/tools/dump_prompt_tag_csvs.py`**. Expanded rows for **prompt adherence** (standard/strict packs, quality, scene, style, misc pairs). **`import` fails** if `data/prompt_tags` is missing. Tests: [`tests/unit/test_content_controls.py`](tests/unit/test_content_controls.py). |
