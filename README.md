...
High-signal additions on top of the same **`train.py` / `sample.py`** engine.

| Area | What shipped |
| :--- | :--- |
| **Part-aware + grounding-aware training** | Hierarchical captions (`caption_global` / `caption_local` / `entity_captions`), optional `grounding_mask`, and foveated crops in [`data/t2i_dataset.py`](data/t2i_dataset.py). Auxiliary grounding and token-coverage losses in [`utils/training/part_aware_training.py`](utils/training/part_aware_training.py), wired into [`train.py`](train.py). |
| **LoRA / DoRA / LyCORIS routing upgrade** | Multi-adapter stacking with normalization, role budgets, and depth-aware stage routing in [`models/lora.py`](models/lora.py). New inference controls in [`sample.py`](sample.py): `path:scale:role`, role budgets, stage policy, and weighted style mixing. |
| **Reproducibility + strictness** | Per-run `run_manifest.json` + `config.train.json` from [`train.py`](train.py), optional strict warnings, and cleaner CLI/config split in [`training/train_cli_parser.py`](training/train_cli_parser.py) and [`training/train_args.py`](training/train_args.py). |
| **Flow / bridge / OT core** | Flow matching, VP bridge auxiliary loss, and OT noise pairing are now first-class training options ([`diffusion/flow_matching.py`](diffusion/flow_matching.py), [`diffusion/bridge_training.py`](diffusion/bridge_training.py), [`utils/training/ot_noise_pairing.py`](utils/training/ot_noise_pairing.py)). |
| **Prompt adherence + control stack** | Expanded prompt-control packs and layout-aware prompt handling for better adherence and style control ([`utils/prompt/content_controls.py`](utils/prompt/content_controls.py), [`utils/prompt/prompt_layout.py`](utils/prompt/prompt_layout.py), `data/prompt_tags/`). |
| **Data + architecture docs that matter** | Curated dataset shortlist in [`docs/HF_DATASET_SHORTLIST.md`](docs/HF_DATASET_SHORTLIST.md), issue-quality guide in [`docs/QUALITY_AND_ISSUES.md`](docs/QUALITY_AND_ISSUES.md), and updated generated architecture diagram via [`scripts/tools/dev/generate_sdx_architecture_diagram.py`](scripts/tools/dev/generate_sdx_architecture_diagram.py) -> `docs/assets/sdx_model_architecture.png`. |
...
| **Prompt scaffolding** | `utils/prompt/content_controls.py` (+ tag data in **`data/prompt_tags/*.csv`** via `content_control_tags.py`): `--one-shot-boost`, `--auto-content-fix`, `--less-ai`, `--anti-ai-pack`, `--human-media`, `--lora-scaffold`, Civitai packs, adherence / quality packs — see **[docs/PROMPT_STACK.md](docs/PROMPT_STACK.md)** |
...
| **CLI preview (no checkpoint)** | `python scripts/tools/preview_generation_prompt.py --prompt "..."` |
